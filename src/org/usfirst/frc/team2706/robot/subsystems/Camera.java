...
public class Camera extends Subsystem {
    ...
    public ArrayList<TargetObject> getVisionData() {
        ...
        /*
         * try { sock.setSoTimeout(5); } catch (SocketException e1) { e1.printStackTrace(); }
         */
        try {
            ...
            try {
                try {
                    if (sock.getInputStream().read(rawBytes) < 0) {
                        System.out.println(
                                        "Something went wrong reading response from TrackerBox2: ");
                        return null;
                    }
                    ...
                    try {
                        ...
                        if (PRINT_STUFF)
                            System.out.println("Target found at: " + pr.ctrX + "," + pr.ctrY
                                            + ", and aspectRatio and boundingArea is: "
                                            + pr.aspectRatio + "," + pr.boundingArea);
    ...
    /*
     * public float savedXaxis = DEFAULT_PAN; public float savedYaxis = DEFAULT_TILT;
     */
    public TargetObject getVisionDataByTarget(int target) { // put in -1 for center target, -2 for
                                                            // right and -3 for left
        ...
        if (pr.size() == 0) {
            ...
            /*
             * TargetObject to = new TargetObject(); to.ctrX = (float) turnXAxis.getPosition();
             * to.ctrY = (float) turnYAxis.getPosition(); return to; }
             */
            return null;
        }
    ...
    public void protectedSetServoAngles(float panValue, float tiltValue) {
        if (turnXAxis.getPosition() > MAX_PAN_RIGHT && panValue > 0
                        || turnXAxis.getPosition() < MAX_PAN_LEFT && panValue < 1) {
